...
app.post('/gerar-pdf', (req, res) => {
  ...
  doc.fontSize(14).font('Helvetica').text('oportunidade que foi autuado pelo suposto cometimento da infração prevista no Art. 173 do Código de Trânsito Brasileiro.', { align: 'left' });
  doc.moveDown();

  doc.fontSize(14).font('Helvetica').text('Pela autoridade de trânsito foi lavrado auto de infração de trânsito de codificação', { continued: true }).fontSize(13).font('Helvetica').text(` ${respostas.autoinfracao}`, { continued: true })
  .text('ocorre que a referida lavratura do auto de infração se deu fora da legalidade, bem como das regulamentações que tratam sobre o assunto.', { align: 'left' });
  doc.moveDown();
...
doc.moveDown();
  ...
  doc.fontSize(14).font('Helvetica').text('A titulo de exemplo verifica-se que o art. 173 do CTB, com base na observância do MBFT, prevê no campo de “Definição de Procedimentos” a necessidade da anotação dos detalhes de toda a ocorrência, a fim de enquadrar a conduta ao fato jurídico, sendo que quando não trazidas as especificações pertinentes à ocorrência, limita sobremaneira o direito de defesa do autuado, haja vista que o administrado não se defende da disposição legal e sim de um fato jurídico embasado numa norma.', { align: 'left' });
  doc.moveDown();

  doc.fontSize(14).font('Helvetica').text('Dito isso, é evidente a necessidade da descrição dos fatos ocorridos e veículos envolvidos quando do suposto cometimento da infração, sob pena de insubsistência do auto lavrado e consequente arquivamento, o que é o caso do presente caso.', { align: 'left' });
  doc.moveDown();
